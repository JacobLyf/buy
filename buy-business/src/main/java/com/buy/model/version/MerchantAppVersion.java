 ...
 * APP 版本
 */
public class MerchantAppVersion extends Model<MerchantAppVersion>{
	private static final long serialVersionUID = 1L;
	
	public static final MerchantAppVersion dao = new MerchantAppVersion();
	...
	public Record getAndroidInfo(String otherViewPath)
	{
		...
		sql.append(" FROM t_merchant_app_version ");
		sql.append(" WHERE 1 = 1 ");
		sql.append(" AND app_type = ? ");
		sql.append(" AND audit_status = ? ");
		sql.append(" ORDER BY CAST(REPLACE(version_no, '.', '') AS SIGNED) DESC ");
		Record result = Db.findFirst(sql.toString(),MerchantAppVersion.TYPE_ANDROID,MerchantAppVersion.AUDIT_STATUS_SUCCESS);
